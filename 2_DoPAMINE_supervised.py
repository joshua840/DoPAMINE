...
#load model
model = Dopamine((40,40,1))
model.compile(loss=sl_loss, optimizer=Adam(lr=lr))
model.summary()

model_test = Dopamine((256,256,1))
model_test.compile(loss=sl_loss, optimizer=Adam(lr=lr))
...
for epoch in range(epochs):
    ...
    model.fit(z_tr.reshape(-1,40,40,1), y, batch_size=batch_size, epochs=epoch+1, callbacks=callback_list, initial_epoch = epoch)
...
del model
model = Dopamine(input_shape=(256,256,1))
model.compile(loss='mean_squared_error', optimizer=Adam(lr=lr))
model.load_weights(weight_path)


AB = model.predict(z_te.reshape(-1,256,256,1), verbose=0, batch_size=4)

X_hat = AB[:,:,:,0]*z_te +AB[:,:,:,1]
for i in range(n_te):
